...
import com.manga.mangareaderapp.adapter.ChapterAdapter;
import com.manga.mangareaderapp.adapter.CommentAdapter;
import com.manga.mangareaderapp.adapter.TagAdapter;
import com.manga.mangareaderapp.model.BannerManga;
import com.manga.mangareaderapp.model.Chapter;
import com.manga.mangareaderapp.model.FavoriteManga;
import com.manga.mangareaderapp.model.Manga;
...
public class MangaDetailsAct extends AppCompatActivity implements ChapterAdapter.OnItemChapterClick {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        favorite = (FavoriteManga) getIntent().getSerializableExtra("favorite");
        manga = (Manga) getIntent().getSerializableExtra("manga");
        bannerManga = (BannerManga) getIntent().getSerializableExtra("bannerManga");
        ...
        btnSend.setOnClickListener(v -> {
            ...
            userRef.child("name").get().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    ...
                    // Thêm phần lấy avatar từ database
                    userRef.child("avatar").get().addOnCompleteListener(taskAvatar -> {
                        if (taskAvatar.isSuccessful()) {
                            String imgUser = taskAvatar.getResult().getValue(String.class);
                            // Tạo đối tượng Rating mới với imgUser
                            if (manga != null)
                                _rate = new Rating(user.getUid(), imgUser, userName, manga.getName(), String.valueOf(rating), cmt);
                            else if (bannerManga != null)
                                _rate = new Rating(user.getUid(), imgUser, userName, bannerManga.getName(), String.valueOf(rating), cmt);
                            else if (favorite != null)
                                _rate = new Rating(user.getUid(), imgUser, userName, favorite.getName(), String.valueOf(rating), cmt);
                            addRateManga();
                            dialog.dismiss();
                            Toast.makeText(this, "Cảm ơn đã đánh giá! Mãi yêu", Toast.LENGTH_SHORT).show();
                        } else {
                            // Xử lý lỗi khi không lấy được avatar
                        }
                    });
                } else {
                    // Xử lý lỗi khi không lấy được tên người dùng
                }
            });
        });
        dialog.show();
    ...
    private void CheckFav() {
        FavoriteManga fav;
        if (manga != null) {
            fav = new FavoriteManga(manga.getName(), manga.getImage(), manga.getCategory(), manga.getDescription(), manga.getAuthor(), manga.getBackdrop(), user.getUid(), manga.getChapters(), manga.getRate());
        } else {
            fav = favorite;
        }
        ...
        reference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot data : snapshot.getChildren()) {
                    ...
                    // Kiểm tra favorite không phải là null và có UID trước khi so sánh
                    if (favorite != null && favorite.getUid() != null && fav != null && fav.getUid() != null) {
        ...
        reference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot data : snapshot.getChildren()) {
                    ...
                    if (rating != null && manga != null) {
                        String ratingName = rating.getMangaName();
                        String mangaName = manga.getName();
                        if (ratingName != null && mangaName != null) {
                            if (ratingName.compareTo(mangaName) == 0) {
